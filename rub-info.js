// رؤوس الأرباع (٨ لكل جزء، ٢٤٠ ربعًا في المصحف كله) — بيانات هيكلية
// (مش نص قرآني، فمفيش تعارض مع القيد الصارم في README بمنع تعديل
// data.js) بتحدد بداية كل ربع بصيغة [رقم السورة, رقم الآية داخل السورة].
// العنصر رقم i (0-based) يمثّل الربع رقم i+1 عالميًا؛ ربع الجزء J رقم
// K (K من ١ لـ٨) هو RUB_STARTS[(J-1)*8 + K - 1].
//
// المصدر الأصلي (قبل التحقق الكامل): مصفوفة hizb_quarter لكل آية من
// مجموعة بيانات malekverse/quran-dataset (CC BY 4.0)، بعد إزالة التكرار
// (كل سجل كان مكررًا مرتين في الملف الأصلي).
//
// ✅ تحقق كامل ومؤكد (٢٤٠/٢٤٠): تمت مطابقة كل نقطة من الـ٢٤٠ في هذا
// الملف — سورة وآية معًا، مش رؤوس الأجزاء الثلاثين بس — مقابل كتاب
// "فتح الرحمن في جداول أرباع القرآن" (د. محمد بكر محمد عبدالهادي،
// شبكة الألوكة، ١٤٣٩هـ)، اللي زوّدنا بيه المستخدم كمصدر مرجعي. المطابقة
// تمت آليًا عبر استخراج جداول الكتاب (pdfplumber) ومقارنتها ببرمجة، لا
// يدويًا — صفر اختلافات. هذا يشمل تصحيح الربع ١٠٦ (الجزء ١٤، الربع
// الثاني = الحجر ٤٩ وليس ٥٠) اللي المستخدم بلّغ عنه سابقًا وأكّده الكتاب
// بنفس القيمة بالظبط.
// Loaded before app.js (see index.html) and exposed as window.RUB_STARTS.
