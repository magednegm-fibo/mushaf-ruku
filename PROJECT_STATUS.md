...
### 1.0.738 — العنكبوت 66 «وليتمتعوا»: قف → ج بني

- الموضع `29:66:4`: نُقل من QIF إلى JEEM.
- اللون النهائي: بني (ج) في مصحف المدينة فقط.

### 1.0.737 — الكهف 71/74/77 «فانطلقا»: قف بني

- `18:71:1` فانطلقا: إضافة QIF.
- `18:74:1` فانطلقا: إضافة QIF.
- `18:77:1` فانطلقا: إضافة QIF (كان موجودًا خامًا).
- اللون النهائي: بني في مصحف المدينة فقط.

### 1.0.736 — طه 21 / العنكبوت 66: قف بني

- `20:21:4` تخف: إضافة QIF.
- `29:66:4` وليتمتعوا: إضافة QIF.
- اللون النهائي: بني في مصحف المدينة فقط.

### 1.0.735 — الزمر 8 «قليلا»: ق أخضر

- الموضع `39:8:30` (قَلِيلٗا): أُزيل من استبعاد QAD_QILA + LA_SILA_OVERRIDE + إضافة.
- اللون النهائي: أخضر في مصحف المدينة فقط.

### 1.0.734 — يوسف 29 «لذنبك»: ج بني

- الموضع `12:29:6` (لِذَنۢبِكِ): أُزيل من استبعاد JEEM + LA_SILA_OVERRIDE + إضافة.
- اللون النهائي: بني في مصحف المدينة فقط.

### 1.0.733 — التوبة 17 «أعمالهم»: ج بني

- الموضع `9:17:14` (أَعۡمَٰلُهُمۡ): أُزيل من استبعاد JEEM + LA_SILA_OVERRIDE + إضافة.
- اللون النهائي: بني في مصحف المدينة فقط.

### 1.0.732 — النساء 140 «غيره»: ز أخضر

- الموضع `4:140:22` (غَيۡرِهِۦٓ): أُزيل من استبعاد ZAY + LA_SILA_OVERRIDE + إضافة.
- اللون النهائي: أخضر في مصحف المدينة فقط.

### 1.0.731 — يونس 16 «به»: ز أخضر

- الموضع `10:16:10` (بِهِۦ في أدراكم به): إضافة ZAY_JAWAZ.
- اللون النهائي: أخضر في مصحف المدينة فقط.

### 1.0.730 — الأعراف 92/151: ج · ز

- `7:92:7` فيها: ج بني (أُزيل استبعاد تعانق + إضافة).
- `7:151:8` رحمتك: ز أخضر (أُزيل استبعاد صلي + LA_SILA_OVERRIDE + إضافة).
- مصحف المدينة فقط.

### 1.0.729 — الأنعام 19/141: قف · ز

- `6:19:7` الله (قل الله): قف بني (أُزيل استبعاد + LA_SILA_OVERRIDE + إضافة).
- `6:141:25` حصاده: ز أخضر (أُزيل استبعاد صلي + LA_SILA_OVERRIDE + إضافة).
- مصحف المدينة فقط.

### 1.0.728 — المائدة 110 «بإذني» ×2: ج بني

- `5:110:42` بإذني (والأبرص بإذني): إضافة JEEM.
- `5:110:46` بإذني (الموتى بإذني): إضافة JEEM.
- اللون النهائي: بني في مصحف المدينة فقط.

### 1.0.727 — البقرة 286 «وارحمنا»: ج بني

- الموضع `2:286:43` (وَٱرۡحَمۡنَآ): تعديل JEEM → QIF (قف بني) في مصحف المدينة.
- اللون النهائي: بني في مصحف المدينة فقط.

### 1.0.726 — البقرة 25/256/285: ق · قف · ز

- `2:25:31` مطهرة: ق أخضر (أُزيل استبعاد + LA_SILA_OVERRIDE + إضافة).
- `2:256:4` الدين: قف بني (أُزيل استبعاد + LA_SILA_OVERRIDE + إضافة).
- `2:285:23` وأطعنا: ز أخضر (إضافة ZAY_JAWAZ).
- مصحف المدينة فقط.

### 1.0.725 — العلق 12 «بالتقوى»: ط أزرق

- الموضع `96:12:3` (بِٱلتَّقۡوَىٰٓ): إضافة TA_MUTLAQ + LA_SILA_OVERRIDE (كان عليها لا).
- اللون النهائي: أزرق في مصحف المدينة فقط.

### 1.0.724 — التين 7: حذف رأس غير الكوفيين · Popup ط وقف مطلق

- حُذف `NON_KUFI` عند `95:7:3` بالكامل.
- يبقى ط أزرق على `95:7:4` + رأس نجمة.
- Popup الافتراضي: «ط — وقف مطلق».

### 1.0.723 — التين 7: إصلاح مفتاح غير الكوفيين للـ Popup [ط]

- كان إصلاحًا مؤقتًا لمفتاح غير الكوفيين؛ أُلغي الرأس في 1.0.724.

### 1.0.722 — التين 7 «بالدين»: ط أزرق + رأس نجمة + غير كوفيين [ط]

- الموضع `95:7:4` (بِٱلدِّينِ): إضافة TA_MUTLAQ + إزالة no-sajawandi.
- مفتاح غير الكوفيين كان خاطئًا (أُصلح في 1.0.723).

### 1.0.721 — الانشقاق 15 «بلى»: ج بني

- الموضع `84:15:1` (بَلَىٰٓ): أُزيل من استبعاد JEEM (تعانق) + إضافة JEEM.
- اللون النهائي: بني في مصحف المدينة فقط.

### 1.0.720 — المرسلات 23 «فقدرنا»: ق أخضر

- الموضع `77:23:1` (فَقَدَرۡنَا): أُزيل من استبعاد QAD_QILA + LA_SILA_OVERRIDE + إضافة.
- اللون النهائي: أخضر في مصحف المدينة فقط.

### 1.0.719 — القلم 43 «ذلة»: ط أزرق

- الموضع `68:43:4` (ذِلَّةٞ): إضافة TA_MUTLAQ.
- اللون النهائي: أزرق في مصحف المدينة فقط.

### 1.0.718 — الممتحنة 10 «لهن»: ج → ط أزرق

- الموضع `60:10:26` (لَهُنَّ): استبعاد ج + إضافة TA_MUTLAQ.
- اللون النهائي: أزرق في مصحف المدينة فقط.

### 1.0.717 — المجادلة 4 «يتماسا»: ط → ج بني

- الموضع `58:4:10` (يَتَمَآسَّا): استبعاد ط + إضافة JEEM.
- اللون النهائي: بني في مصحف المدينة فقط.

### 1.0.716 — الجاثية 24 «الدهر»: ط → ج بني

- الموضع `45:24:12` (ٱلدَّهۡرُ): استبعاد ط + إضافة JEEM.
- اللون النهائي: بني في مصحف المدينة فقط.

### 1.0.715 — الزخرف 45 «رسلنا»: ق أخضر

- الموضع `43:45:7` (رُّسُلِنَآ): إضافة QAD_QILA.
- اللون النهائي: أخضر في مصحف المدينة فقط.

### 1.0.714 — غافر 68 «يميت»: ط → ج بني

- الموضع `40:68:4` (وَيُمِيتُ): استبعاد ط + إضافة JEEM.
- اللون النهائي: بني في مصحف المدينة فقط.

### 1.0.713 — غافر 67 «شيوخا»: ط → ج بني

- الموضع `40:67:20` (شُيُوخٗا): استبعاد ط + إضافة JEEM.
- اللون النهائي: بني في مصحف المدينة فقط.

### 1.0.712 — غافر 28: كذبه ط→ج · يعدكم ج→ط

- `40:28:24` كذبه (فعليه كذبه): استبعاد ط + إضافة ج بني.
- `40:28:31` يعدكم (الذي يعدكم): استبعاد ج + إضافة ط أزرق.

### 1.0.711 — الأحزاب 60 «قليلا»: إصلاح تلوين ج بني

- السبب: الموضع محجوب بطبقة لا/صلي (`SILA` في `DEFAULT_MARK_LA_SILA_BLOCK`).
- الإصلاح: `DEFAULT_MARK_LA_SILA_OVERRIDES['33:60:19'] = { JEEM: true }`.
- اللون النهائي: بني في مصحف المدينة فقط.

### 1.0.710 — الأحزاب 60 «قليلا»: ج بني (إضافة أولية)

- الموضع `33:60:19` (قَلِيلٗا): إضافة JEEM — لم يظهر لأن صلي كانت تحجبها (أُصلح في 1.0.711).

### 1.0.709 — الأحزاب 61 «ملعونين»: ج بني

- الموضع `33:61:1` (مَّلۡعُونِينَ): أُزيل من استبعاد JEEM (تعانق) + إضافة JEEM.
- اللون النهائي: بني في مصحف المدينة فقط.

### 1.0.708 — السجدة 12 «ربهم»: ط أزرق

- الموضع `32:12:8` (رَبِّهِمْ في عند ربهم): إضافة TA_MUTLAQ.
- اللون النهائي: أزرق في مصحف المدينة فقط.

### 1.0.707 — لقمان 32 «الدين»: ج بني + رأس غير الكوفيين [ج]

- الموضع `31:32:9` (ٱلدِّينَ في مخلصين له الدين): إضافة JEEM + LA_SILA_OVERRIDE.
- `NON_KUFI` 31:32:8: لا/أخضر → ج/بني → Popup «رأس آية لغير الكوفيين [ ج ]».
- اللون النهائي: بني في مصحف المدينة فقط.

### 1.0.706 — كاش HTTP/CDN لبقية الملفات

- `_headers`: ثلاث طبقات — مسار التحديث (sw/index/version/manifest) بدون كاش؛ JS/CSS/data بدون كاش؛ fonts/icons بـ immutable سنة.
- `sw.js`: network-first للتنقّل والملفات الديناميكية يستخدم `cache: 'no-cache'` حتى لا يُعتبر رد CDN القديم «شبكة».

### 1.0.705 — توحيد رقم الإصدار (sync-version)

- `tools/sync-version.js`: يقرأ `version.js` ويحدّث package.json + manifest.json + PROJECT_STATUS.md + README.md.
- `tests/version-sync-regression.js`: يمنع اختلاف الأرقام (8 فحوصات بما فيها sw.js).
- أوامر npm: `version:sync` و`version:check`.
...
- `5:20:15` مُّلُوكٗا: أُبقي اعتماد `QAD_QILA` ليظهر «قد قيل» بالأخضر في مصحف المدينة.
