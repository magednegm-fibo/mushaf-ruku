...
**الإصدار الحالي:** 1.0.704  
**آخر تحديث:** 2026-09-14  
**ملاحظة توحيد:** هذا الرقم مطابق الآن لـ `version.js` و`manifest.json` وأعلى `README.md`. إدخالات هذا الملف أدناه موثّقة حتى 1.0.627 (مع إدخال منفرد خارج الترتيب لـ 1.0.628 داخل قسم "Change Log" القديم أسفل الملف)؛ الإصدارات 1.0.629–1.0.636 غير موثّقة هنا كتغييرات منفصلة (راجع تعليقات الكود في `prayer.js` و`tests/prayer-reliability-regression.js`)، والإصدارات 1.0.637–1.0.657 موثّقة في سجل التغييرات بـ `README.md`.
...
### 1.0.704 — القصص 56 «يشاء»: ط → ج بني

- الموضع `28:56:10` (يَشَآءُ): استبعاد ط + إضافة ج.
- اللون النهائي: بني في مصحف المدينة فقط.

### 1.0.703 — الشعراء 49 «تعلمون»: ط أزرق + رأس غير الكوفيين [ط]

- الموضع `26:49:14`: إضافة ط + LA_SILA_OVERRIDE.
- `NON_KUFI` 26:49:13: لا/أخضر → ط/أزرق → Popup «رأس آية لغير الكوفيين [ ط ]».

### 1.0.702 — الفرقان 59 «العرش»: إضافة ج بني

- الموضع `25:59:13` (ٱلۡعَرۡشِ): كان مستبعدًا لتعانق؛ أُزيل من الاستبعاد + إضافة JEEM.
- اللون النهائي: بني في مصحف المدينة فقط.

### 1.0.701 — النور: ثلاثة مواضع

- `24:17:9` مؤمنين: إضافة ج بني + إزالة من no-sajawandi → رأس نجمة.
- `24:45:16` رجلين: إضافة ج بني.
- `24:53:7` ليخرجن: ج → ط أزرق.

### 1.0.700 — الحج 53 و72: ج → ط أزرق

- `22:53:11` قلوبهم (والقاسية قلوبهم): استبعاد ج + إضافة ط.
- `22:72:27` كفروا (الذين كفروا): استبعاد ج + إضافة ط.
- اللون النهائي: أزرق في مصحف المدينة فقط.

### 1.0.699 — السجدة/الشورى/الأحقاف/ق: فواتح السور

- `32:1:1` الم: إضافة ج بني + إزالة من no-sajawandi → رأس نجمة.
- `42:1:1` حم: إضافة ج بني + رأس نجمة.
- `46:1:1` حم: إضافة ج بني + رأس نجمة.
- `50:1:1` قٓ: ج → قف بني (استبعاد JEEM + إضافة QIF).

### 1.0.698 — طه 86: رموز ج/ط على رأس غير الكوفيين في الـ Popup

- `20:86:5` أسفا: SYM=ج، لون brown → Popup «رأس آية لغير الكوفيين [ ج ]».
- `20:86:13/12` حسنا: SYM=ط، لون blue → Popup «رأس آية لغير الكوفيين [ ط ]».

### 1.0.697 — طه: سبعة تعديلات

- `20:1:1` طه: حذف ج + رأس دائرة (no-sajawandi).
- `20:74:8` جهنم: ج → ط أزرق.
- `20:86:6` أسفا: إضافة ج بني + إزالة لا/أخضر عن رأس غير الكوفيين + LA_SILA override.
- `20:86:13` حسنا: إضافة ط أزرق + إزالة لا/أخضر عن رأس غير الكوفيين + LA_SILA override.
- `20:88:10` رأس غير الكوفيين (موسى): لا أخضر (كان عارٍ).
- `20:88:12` فنسي: إضافة ط أزرق + رأس نجمة (إزالة من no-sajawandi).
- `20:108:6` له: ط → ج بني.

### 1.0.696 — مريم: أربعة مواضع

- `19:1:1` كهيعص: حذف ج (+ قف) → بلا علامة؛ إضافة 19:1 إلى no-sajawandi-heads → رأس **دائرة**.
- `19:29:2` إليه: ط → قف بني.
- `19:71:4` واردها: ط → ج بني.
- `19:94:4` عدّا: إضافة ط أزرق؛ إزالة 19:94 من no-sajawandi-heads → رأس **نجمة**.

### 1.0.695 — الكهف 29 «الشراب»: إضافة ط أزرق

- الموضع `18:29:26` (ٱلشَّرَابُ): إضافة وقف مطلق ط عبر `DEFAULT_MARK_MANUAL_ADDITIONS['TA_MUTLAQ']`.
- اللون النهائي: أزرق في مصحف المدينة فقط.

### 1.0.694 — الإسراء 61 «إبليس»: إضافة ط أزرق

- الموضع `17:61:8` (إِبۡلِيسَ): إضافة وقف مطلق ط عبر `DEFAULT_MARK_MANUAL_ADDITIONS['TA_MUTLAQ']`.
- اللون النهائي: أزرق في مصحف المدينة فقط.

### 1.0.693 — التوبة 36 «أنفسكم»: ط → قف بني

- الموضع `9:36:25` (أَنفُسَكُمۡ): استبعاد ط + إضافة قف.
- اللون النهائي: بني (وقف مستحب قف) في مصحف المدينة فقط.

### 1.0.692 — النحل 126 «به»: ج → ط أزرق

- الموضع `16:126:7` (بِهِۦ): استبعاد ج + إضافة ط.
- اللون النهائي: أزرق (وقف مطلق ط) في مصحف المدينة فقط.

### 1.0.691 — هود 23 «الجنة»: ط → ج بني

- الموضع `11:23:11` (ٱلۡجَنَّةِ): استبعاد ط + إضافة ج.
- اللون النهائي: بني (وقف جائز ج) في مصحف المدينة فقط.

### 1.0.690 — يونس 22: رمز ج على رأس غير الكوفيين في الـ Popup

- `NON_KUFI_HEADS_SYM_*` لـ 10:22 (عثماني 10:22:33 / إندوباك 10:22:34): من عارٍ إلى **ج**.
- `NON_KUFI_HEADS_*` اللون: **brown**.
- الـ Popup عند الضغط المطوّل: «رأس آية لغير الكوفيين [ ج ]».
- تلوين كلمة الدين ج بني يبقى كما في 1.0.689.

### 1.0.689 — يونس: ثلاثة مواضع ج بني + إزالة لا عن رأس 10:22

- `10:15:25` نفسي (تلقائي نفسي): إضافة ج بني.
- `10:22:34` الدين (له الدين): إضافة ج بني + `LA_SILA_OVERRIDES` لتجاوز لا؛ جداول `NON_KUFI_HEADS_*` / `NON_KUFI_HEADS_SYM_*` (عثماني 10:22:33 / إندوباك 10:22:34) من green/لا إلى رأس عارٍ (`""`).
- `10:27:24` النار (أصحاب النار): ط → ج — استبعاد ط + إضافة ج. اللون النهائي: بني.
- مصحف النسخ بلا تغيير في سلوك التلوين السجاوندي الافتراضي.

### 1.0.688 — التوبة: ز→ج على الله + ج على لهم

- `9:30:21` الله (قاتلهم الله): ز → ج — استبعاد من `DEFAULT_MARK_MANUAL_EXCLUSIONS['ZAY_JAWAZ']` + إضافة `DEFAULT_MARK_MANUAL_ADDITIONS['JEEM']`. اللون النهائي: بني.
- `9:74:29` لهم (خيرا لهم): إضافة وقف جائز ج بني عبر `DEFAULT_MARK_MANUAL_ADDITIONS['JEEM']`.
- مصحف النسخ بلا تغيير.

### 1.0.687 — الأعراف 1 «المص»: ج بني + رأس نجمة

- الموضع `7:1:1` (ٱلٓمٓصٓ): إضافة تلوين ج (`DEFAULT_MARK_MANUAL_ADDITIONS['JEEM']`).
- اللون النهائي: بني (وقف جائز ج) في مصحف المدينة فقط.
- الآية كلمة واحدة (مقطعات) وكانت في `data/no-sajawandi-heads.json` → أُزيلت منها (4038 موضعًا بدل 4039) فتتحول رأس الآية من دائرة إلى نجمة ثمانية.
- لا تغيير على مصحف النسخ ولا جداول non-kufi.

### 1.0.686 — ثلاثة مواضع في الأنعام

- `6:89:6` والنبوة: ط → ج
- `6:137:12` دينهم: ج → ط
- `6:137:17` فعلوه: ط → صلي (إزالة تلوين ط؛ الموضع يحمل U+06D6 وأُضيف إلى SILA_POSITIONS)

### 1.0.685 — ملوكا: تجاوز صلي لتلوين ق

- `5:20:15` كانت لا تزال محجوبة بطبقة لا/صلي لأن الموضع في SILA_POSITIONS؛ أُضيف `LA_SILA_OVERRIDES['5:20:15'].QAD_QILA` ليظهر الأخضر فعليًا.

### 1.0.684 — المائدة 20 ملوكا: ق أخضر

- `5:20:15` مُّلُوكٗا: كانت مستبعدة من تلوين ق؛ أُزيل الاستبعاد ليظهر «قد قيل» بالأخضر في مصحف المدينة.

### 1.0.683 — أربعة مواضع في النساء

- `4:11:42` الثلث: ط → ج
- `4:25:34` أخدان: ط → ج
- `4:83:9` به: ج → ط
- `4:135:14` والأقربين: ط → ج

### 1.0.682 — آل عمران 172 القرح: ط أزرق

- `3:172:9` ٱلۡقَرۡحُ: كانت مستبعدة من تلوين ط بسبب التعانق؛ أُزيل الاستبعاد بطلب مباشر ليظهر وقف مطلق أزرق في مصحف المدينة.

### 1.0.681 — ثلاثة مواضع وقف (بقرة)

- `2:187:42` الفجر: ط → ص (استبعاد ط + إضافة ص).
- `2:222:3` المحيض: لا → ط (إضافة ط + تجاوز لا/صلي عبر LA_SILA_OVERRIDES؛ الإضافات اليدوية صارت تحترم نفس الاستثناءات).
- `2:255:5` هو: إضافة ج بني.
- ملاحظة: طلب المستخدم «2:223:3 المحيض» يطابق نص الآية 222 الكلمة 3 (المحيض)، لا 223:3 (لكم).

### 1.0.680 — التحريم 2 «أيمانكم»: ط → ج

- الموضع `66:2:6` (أَيۡمَٰنِكُمۡ): استبعاد من تلوين ط (`DEFAULT_MARK_MANUAL_EXCLUSIONS['TA_MUTLAQ']`) وإضافة تلوين ج (`DEFAULT_MARK_MANUAL_ADDITIONS['JEEM']`).
- اللون النهائي: بني (وقف جائز ج) بدل الأزرق (وقف مطلق ط).
- لا تغيير على جداول non-kufi أو مصحف النسخ.

### 1.0.679 — حذف علامة الوقف عن «الألباب» (الطلاق 10) بسبب التعانق

- الموضع `65:10` كلمة «ٱلۡأَلۡبَٰبِ» (فهرس عثماني 8 / إندوباك 9): كانت تحمل ج + تعانق.
- جداول `NON_KUFI_HEADS_*` / `NON_KUFI_HEADS_SYM_*`: من brown/ج إلى رأس عارٍ (`""`) — بلا علامة وقف ولا تلوين، مع الإبقاء على رأس غير الكوفيين.
- الاستبعاد اليدوي `DEFAULT_MARK_MANUAL_EXCLUSIONS['JEEM']['65:10:9']` وSILA كانا يمنعان تلوين ج من المسار الافتراضي؛ تنظيف SYM يمنع أي نجمة عبر مسار الـ Override أيضًا.
- الطرف الآخر من التعانق (`65:10:11`) يبقى محسومًا سابقًا (قف).

### 1.0.678 — Override وقف فعلي: 65:2:23 لا→ط

- جدول `NON_KUFI_HEADS_SYM_*`: الموضع `65:2:23` من «لا» إلى «ط».
- جدول الألوان `NON_KUFI_HEADS_*`: من green إلى blue (لون ط).
- `readerManager.js`: مسار الـ shifted marks لرأس غير الكوفيين يقدِّم رمز الـ Override النهائي على سياسة لا/صلي الأصلية — فتُلوَّن الكلمة ونجمة الوقف فعليًا، لا الـ Popup فقط.
- مصحف النسخ (Indopak) بلا تغيير في السلوك (الرؤوس غير الكوفية للعُثماني فقط).
- لا جدول Override جديد؛ الاعتماد على الجداول الحالية في كل المسارات (علامة، لون، رأس، Popup، Tooltip).

### 1.0.677 — استثناء 11 موضع صلي×سجاوندي (جدول المراجعة)

**الطلب:** إضافة المواضع الـ11 الظاهرة في صورة جدول المراجعة إلى قائمة
استثناءات الوقف السجاوندي في مصحف المدينة. هذه المواضع عليها علامة «صلي»
في مصحف النسخ، وكانت مستبعدة تلقائيًا عبر `DEFAULT_MARK_LA_SILA_BLOCK`،
ويُراد استثناؤها من القاعدة وتلوينها في مصحف المدينة بدرجة وقف النسخ
(ج بني / ز أخضر / ق أخضر).

**التعديل:**
1. إزالة المواضع الـ11 من `DEFAULT_MARK_MANUAL_EXCLUSIONS` لأنواعها.
2. إضافتها إلى `DEFAULT_MARK_LA_SILA_OVERRIDES` لتجاوز طبقة لا/صلي العامة.

| الموضع | العلامة | اللون |
|--------|---------|------|
| 2:133:24 | ج | بني |
| 4:141:13 | ز | أخضر |
| 9:52:18 | ز | أخضر |
...
**اختبار:** `tests/sila-override-batch-676-regression.js` — 11/11 نجح.

### 1.0.675 — المزمل 73:20:48 — اعتماد وقف ز (استثناء تقاطع ز×صلي)

الكلمة «ٱللَّهِۖ» في الآية 20 من سورة المزمل («وَءَاخَرُونَ يُقَٰتِلُونَ فِي سَبِيلِ ٱللَّهِ») تحمل صلى فعليًا
في مصحف المدينة (ۖ) وعليها أيضًا علامة ز (وقف جائز) في مصحف النسخ. كانت مُستبعدة تلقائيًا من التلوين
عبر `DEFAULT_MARK_LA_SILA_BLOCK` ضمن سياسة استبعاد تقاطع ز×صلي العامة، والموضع كان مذكورًا صراحة
أيضًا في `DEFAULT_MARK_MANUAL_EXCLUSIONS['ZAY_JAWAZ']` (`'73:20:48': true`).

طلب مباشر: اعتماد هذا الوقف ز عند هذا الموضع تحديدًا، وتلوينه أخضر بدرجة الوقف ز في مصحف المدينة —
بنفس نمط استثناء 29:60:9 (1.0.665):
- حذف `'73:20:48': true` من `DEFAULT_MARK_MANUAL_EXCLUSIONS['ZAY_JAWAZ']` في `readerManager.js`.
- إضافة `'73:20:48': { ZAY_JAWAZ: true }` إلى `DEFAULT_MARK_LA_SILA_OVERRIDES` لتجاوز طبقة
  `DEFAULT_MARK_LA_SILA_BLOCK` لهذا الموضع فقط.
- التلوين يتبع تلقائيًا عبر `DEFAULT_ZAY_JAWAZ_WORDS = buildDefaultWordMap('ZAY_JAWAZ')` — لا حاجة
  لأي تعديل في style.css (نفس اللون الأخضر `#2E7D32` المستخدم لبقية مواضع ز).
- اختبار انحدار جديد: `tests/sila-override-batch-675-regression.js` (1/1 نجح).
- رفع رقم الإصدار إلى 1.0.675 في `version.js`، `manifest.json`، `package.json`، وهذا الملف و`README.md`.

**الحالة:** ✅ مكتمل. كل ملفات الاختبار (23 ملفًا) نجحت بعد التعديل.

---

### 1.0.674 — إصلاح offline PWA على Cloudflare Workers (جذري)

Root cause: طلبات التنقّل offline كانت قد تُرجع `undefined` من `respondWith` عند عدم تطابق مفتاح الكاش
(`/` مقابل `/index.html`)، فيظهر ERR_FAILED على workers.dev. كذلك تسجيل SW كان يتأخر حتى `window.load`.

التغييرات:
- مسار تنقّل صريح: شبكة أولًا، ثم shell من الكاش، ولن يُعاد `undefined` أبدًا.
- نسخ `index.html` تحت عدة مفاتيح كاش عند التثبيت وكل تنقّل ناجح.
- تسجيل مبكّر للـ SW من `index.html` + `scope: './'`.
- إضافة `waqf-positions.js` / `sila-positions.js` للقائمة precache.

### 1.0.673 — تحصين العمل دون إنترنت على Cloudflare Workers

- تثبيت الكاش ملفًا ملفًا بدل `cache.addAll` (كان فشل رابط واحد يلغي التثبيت بالكامل).
- احتياطي تنقّل offline أقوى (`/`, `index.html`, `scope`) لمعالجة اختلاف مطابقة الطلبات على workers.dev.

### 1.0.672 — الأعراف 69: إلغاء ج على جعلكم + اعتماد ج على بصطة

حسب مصحف النسخ المطبوع:
- `7:69:13` جَعَلَكُمۡ: استبعاد ج من التلوين
- `7:69:22` بَصۜۡطَةٗ: إضافة ج يدوية (بني)

### 1.0.671 — اعتماد مواضع 1–15 عدا 4 و11

مستبعدان: `6:141:25` (#4)، `12:29:6` (#11).
معتمدون (13): 2:24:11 ج، 2:221:28 ج، 5:41:62 ج، 6:146:26 ز، 7:154:7 ز، 7:179:7 ز، 7:196:6 ز، 11:16:8 ز، 11:46:10 ق، 17:73:10 ق، 18:18:9 ق، 19:6:5 ق، 21:32:4 ج.

### 1.0.670 — إعادة نشر دفعة 16–33 (تأكيد تلوين 22:36:9 ق)

تحقق برمجي: `DEFAULT_QAD_QILA_WORDS['22:36']` يتضمن الفهرس 8 (الكلمة 9 خَيْرٞ).
رفع رقم الإصدار لإجبار تحديث كاش Service Worker بعد بلاغ عدم ظهور اللون.

### 1.0.669 — اعتماد تلوين مواضع 16–33 (مرجّح ≥1 قارئ)

بعد اعتماد المستخدم بالكامل: تطبيق ألوان السجاوندي على المواضع عبر `DEFAULT_MARK_LA_SILA_OVERRIDES`.

ق أخضر: 22:36:9، 23:18:8، 23:72:6، 24:33:21، 25:65:7، 28:19:18  
ص أخضر: 24:22:15  
ز أخضر: 35:3:19، 35:8:15  
ج بني: 38:6:8، 40:64:17، 47:26:12، 50:16:8  
(مع الإبقاء على معتمدات سابقة في نفس النطاق: 33:19:2، 33:23:15، 38:5:4، 38:65:4، 42:31:5)

### 1.0.668 — الحديد 57:19: اعتماد ق على الصديقون + حذف ط من ربهم

- `57:19:7` (الصِّدِّيقُونَ): تلوين ق أخضر عبر `DEFAULT_MARK_LA_SILA_OVERRIDES` (كانت مستبعدة لصلى).
- `57:19:10` (رَبِّهِمۡ في «عند ربهم»): استبعاد ط من التلوين (`DEFAULT_MARK_MANUAL_EXCLUSIONS['TA_MUTLAQ']`) لمخالفتها قراءة القراء.

### 1.0.667 — تلوين 5 مواضع اتفق عليها القراء الأربعة (وقف واضح)

مواضع صلي نسخ × صلي مدينة (ۖ) بلا تعانق، وحكم التحليل الصوتي التقديري
= وقف واضح عند الحصري والمنشاوي وعبدالباسط وجبريل معًا.
...
أُزيلت من `DEFAULT_MARK_MANUAL_EXCLUSIONS` وأُضيفت إلى `DEFAULT_MARK_LA_SILA_OVERRIDES`.

### 1.0.666 — استثناء 12 موضع صلي×سجاوندي لها علامة مدينة غير صلي

**الطلب:** اعتماد تلوين درجة وقف النسخ (ج بني / ز وق أخضر) في مصحف المدينة
لـ12 موضعًا كانت مهمَلة بسبب صلي، لكنها تحمل علامة وقف عثمانية غير صلي
(ۚ) في نص المدينة.

**التعديل:**
1. إزالة المواضع الـ12 من `DEFAULT_MARK_MANUAL_EXCLUSIONS` لأنواعها.
2. إضافتها إلى `DEFAULT_MARK_LA_SILA_OVERRIDES` لتجاوز طبقة لا/صلي العامة.
...
**اختبار:** `tests/sila-override-batch-666-regression.js` — 50/50.

### 1.0.665 — استثناء ز×صلي عند 29:60:9 (العنكبوت، "وَإِيَّاكُمۡ")

- عليها في مصحف النسخ علامتان معًا: صلي وز. حسب السياسة العامة (أي ز
  تتقاطع مع صلي → استبعاد بلا استثناءات فردية) كانت مستبعدة تلقائيًا.
- استثناء صريح موثّق لهذا الموضع تحديدًا (اعتماد الوقف ز، تلوين أخضر)،
  بنفس نمط استثناء `3:30:9` (JEEM) السابق. لازم خطوتان معًا (نفس الدرس
  من 3:30:9):
  1. حذف `'29:60:9': true` من `DEFAULT_MARK_MANUAL_EXCLUSIONS['ZAY_JAWAZ']`.
  2. إضافة `'29:60:9': { ZAY_JAWAZ: true }` إلى `DEFAULT_MARK_LA_SILA_OVERRIDES`
     (الطبقة الأسبق لسياسة لا/صلي العامة — إزالة الإدخال اليدوي وحده غير
     كافية، تظل مستبعدة عبرها ما لم يُضَف استثناء صريح هنا أيضًا).
- تعديل في `readerManager.js` فقط؛ لا تغيير في `waqf-positions.js` أو
  `sila-positions.js` أو `data.js`.
- تحقّق مباشر: `buildDefaultWordMap('ZAY_JAWAZ')['29:60']` يتضمن الآن
  الفهرس `8` (الكلمة التاسعة، صفر-محسوب).

### 1.0.657 — إصلاح تلوين خاطئ لموضع ط في الحشر 59:7

- حذف مدخل خاطئ `'59:7:23'` من `DEFAULT_MARK_MANUAL_ADDITIONS['JEEM']` في `readerManager.js`.
- الموضع (منكم) TA_MUTLAQ أصلاً في `WAQF_POSITIONS`؛ التعارض مع إضافة JEEM اليدوية كان يجعله يُعرض بنيًا بدل أزرق.
- تعديل سطر واحد فقط؛ لا تغيير في Architecture أو CSS أو Long Press.

### 1.0.656 — بوصلة القبلة: Close عند مغادرة التبويب (لا Pause/Resume)

- `onTabHidden` → `stopCompass()` كاملًا.
- حذف resume window و`startCompass({ resume })` من `onTabShown`.
- اختبار: `tests/compass-tab-lifecycle-regression.js`.

### 1.0.655 — بوصلة القبلة: إغلاق عند مغادرة تاب الصلاة / إغلاق الدليل

- `reader-guide.js`: MutationObserver على `#waqfGuidePanel` يستدعي `Prayer.onTabHidden` عند الإخفاء والتبويب نشط = الصلاة (يغطي زر الرجوع / closeTopmostOverlay).

### 1.0.654 — تفسير الركوع: فصل نص التفسير عن CATT warm-up

- `prefetchTafsirTexts`: preload نص فقط (متوازٍ) لـ current / next / prev — مستقل عن CATT.
- `prefetchNeighbors`: يملأ كاش النص فورًا؛ سلسلة CATT تبقى خلفية ولا تحجب العرض.
- `loadCurrentRuku` + `renderCachedRuku`: عرض فوري بدون skeleton عند اكتمال الكاش.

### 1.0.653 — تفسير الركوع: مسار كاش فوري

- `allAyahsCached` + `renderCachedRuku` لتجنب وميض skeleton عند وجود البيانات.

### 1.0.652 — إصلاح offline لـ prayer.js مع الإبقاء على Lazy load

- `prayer.js` عاد إلى DYNAMIC_ASSETS (precache) لأنه Core offline (صلاة/قبلة).
- التحميل في الصفحة ما زال كسولًا عبر ensurePrayer.
- اختبارات: `tests/prayer-offline-precache-regression.js`, `e2e/prayer-offline.spec.js`.

### 1.0.642 — تحسين سرعة التشغيل الأول (Lazy load)

- تحميل كسول: radio-player / prayer / reader-tafsir (+ قاموس التشكيل و tts-diacritizer) عند أول استخدام.
- إخراج غير-Core من precache؛ prayer أُصلح في 1.0.652.
- إضافة `lazy-loader.js`.

### 1.0.641 — إضافة اختبارات Playwright E2E

- إضافة مجلد `e2e/` مع اختبارات متصفح حقيقية (Playwright) للسيناريوهات الأساسية والمتقدمة.
- إضافة `package.json` و`playwright.config.js`.
- لا تغييرات وظيفية في التطبيق.

### 1.0.640 — توحيد معلومات الإصدار والتوثيق

- إصلاح تعارض أرقام الإصدار عبر ملفات التوثيق: كان `README.md` يحمل ملاحظة قديمة تقول إن الإصدار الحالي 1.0.515 رغم أن أعلى الملف و`version.js`/`manifest.json` عند 1.0.639 — حُذفت الملاحظة القديمة واستُبدلت برقم صحيح.
- كان هذا الملف (`PROJECT_STATUS.md`) يحمل ثلاثة أرقام إصدار متضاربة في نفس الوقت: 1.0.627 في الرأس، 1.0.174 في قسم "Project Metrics"، وإدخال 1.0.628 مُقحَم فوق سجل "Change Log" التاريخي القديم (الذي يقفز بعده مباشرة إلى 1.0.191). أُضيفت ملاحظات توضيحية عند كل موضع بدل حذفها، حتى لا تُفقَد أي معلومة تاريخية.
- لا تغييرات في الكود أو السلوك الوظيفي للتطبيق — توثيق فقط.
...
*(ملاحظة: هذا القسم سجل تاريخي قديم مرتّب تنازليًا من حقبة 1.0.1xx–1.0.1xx تقريبًا؛ إدخال 1.0.628 أدناه أُضيف لاحقًا فوقه دون إعادة ترتيب الباقي، وليس استمرارًا زمنيًا لما يليه. للسجل الزمني الصحيح للإصدارات الحديثة (1.0.608 وما بعده) راجع قسم "Session Handoff" أعلى هذا الملف، ولإصدارات 1.0.637–1.0.639 راجع `README.md`.)*

### 1.0.628
- Salah tab, Qibla section: added a static, permanently visible tip line directly after "اتجاه القبلة" summary — `حرّك الهاتف على شكل 8 عدة مرات لمعايرة البوصلة.` — matching the requested reference screenshot. Presentational only (new `.prayer-calib-tip` rule + night-mode color), no JS logic attached, compass/Qibla math untouched.
...
**Current Version:** 1.0.639 *(هذا الرقم كان متروكًا عند 1.0.174 من جلسة قديمة جدًا — صُحِّح ليطابق `version.js`/`manifest.json`؛ الإحصاءات أدناه (اختبارات الوقف/البحث) تعود لتلك الحقبة ولم يُعَد التحقق منها منذ ذلك الحين)
